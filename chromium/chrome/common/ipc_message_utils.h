 ...
 public:
  ...
  MessageWithReply(int32 routing_id, uint16 type,
      ...
      : SyncMessage(routing_id, type, PRIORITY_NORMAL,
                    new ParamDeserializer<ReplyParam>(reply)) { //ParamDeserializer继承自MessageReplyDeserializer，相当于模板子类
    WriteParam(this, send);                                     //从基类继承虚函数并实现之，这样就能针对不同的tuple采用writeparam
  }
