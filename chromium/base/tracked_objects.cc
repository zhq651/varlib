...
// This may be called from another thread.复制 birth_map
...
// This may be called from another thread.复制 death_map
...
#ifdef OS_WIN
void ThreadData::RunOnAllThreads(void (*function)()) {//发送function到每一个messageloop中执行该函数
...
//根据初始化的时候的remaining_count_判断是不是最后一次调用，是的话就delete自己
