...
// table.
long ThreadLocalStorage::tls_key_ = TLS_OUT_OF_INDEXES;//static
...
long ThreadLocalStorage::tls_max_ = 1;//static
...
bool ThreadLocalStorage::Slot::Initialize(TLSDestructorFunc destructor) {
  if (tls_key_ == TLS_OUT_OF_INDEXES || !TlsGetValue(tls_key_))//检查ThreadLocalStorage是否已经初始化,tls_key_是ThreadLocalStorage的静态成员
