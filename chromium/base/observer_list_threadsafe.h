...
class ObserverListThreadSafe
 ...
 public:
  ...
  void Notify(Method m) {
    UnboundMethod<ObserverType, Method, Tuple0> method(m, MakeTuple());//到NotifyWrapper才会执行用户回调
    Notify<Method, Tuple0>(method);//invoke private notify
  }

  template <class Method, class A>
  void Notify(Method m, const A &a) {//const 引用，如果是a是指针则a是不可变不代表a指向的对象不可变
    UnboundMethod<ObserverType, Method, Tuple1<A> > method(m, MakeTuple(a));
    Notify<Method, Tuple1<A> >(method);//invoke private notify
  }
 ...
 private:
      ...
      loop->PostTask(FROM_HERE,
          NewRunnableMethod(this,
              &ObserverListThreadSafe<ObserverType>::
				 ...
				 //改task执行参数2  this->NotifyWrapper(list,method) 

				 //PS:NotifyWrapper是私有,在NewRunnableMethod->RunnableMethod->DispatchToMethod中能以(obj->*method)(arg.a, arg.b)
				 //方式调用是因为传递的是一个类私有的成员函数指针给NewRunnableMethod,
				 //通过对象指针以及指向类私有成员函数的指针是能够在外部显示调用该private method的
  ...
  void NotifyWrapper(ObserverList<ObserverType>* list,
    ...
    // Check that this list still needs notifications.
    {
      ...
      // notification.
      if (it == observer_lists_.end()|| /*可能已经被移除*/ 
	  	  it->second != list)/*可能删除之后马上又添加进来，list指针变为野指针*/
        return;
    }

    {
      ...
      while ((obs = it.GetNext()) != NULL)
        method.Run(obs);//用户绑定的参数封装在method内部
    }
