 ...
 private:
  ...
  class TimerTask : public BaseTimer_Helper::TimerTask {
   public:
    ...
    virtual void Run() {
      if (!timer_)  // timer_ is null if we were orphaned.注意此处判断是否为null很重要，为null说明外层basetimer已经于自己断绝联系
        return;
      if (kIsRepeating)//重复计时器
        ResetBaseTimer();
      else
        ClearBaseTimer();
   ...
   private:
    ...
    void ClearBaseTimer() {
      if (timer_) {
        ...
        if (self->delayed_task_ == this)//指向外层的BaseTimer如果与自己有联系则断绝联系
