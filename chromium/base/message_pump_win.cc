...
//重要关键:    之前有工作的话have_work_==1直接返回，*确保有多个任务task同时 posttask到messageloop之后，
//win32 消息队列中只有一个kMsgHaveWork*
...
void MessagePumpForUI::ScheduleDelayedWork(const Time& delayed_work_time) {
  ...
  //
  delayed_work_time_ = delayed_work_time;//通知消息磊下一个超时时间发生在delayed_work_time

  int delay_msec = GetCurrentDelay();//计算现在距离delayed_work_time的微妙数，用于SetTimer
  DCHECK(delay_msec >= 0);
  ...
  SetTimer(message_hwnd_, reinterpret_cast<UINT_PTR>(this), delay_msec, NULL);//在delay_msec发送一个消息唤醒
}
...
void MessagePumpForUI::DoRunLoop() {
  ...
  for (;;) {
    ...
    bool more_work_is_plausible = ProcessNextWindowsMessage();//作用是优先处理win32内核消息，再处理我们要干的活,返回值是true说明在消息队列里面有我们要干的活
...
bool MessagePumpForUI::ProcessMessageHelper(const MSG& msg) {
  ...
  // While running our main message pump, we discard kMsgHaveWork messages.暂时丢弃kMsgHaveWork消息
  if (msg.message == kMsgHaveWork && msg.hwnd == message_hwnd_)
    return ProcessPumpReplacementMessage();
//系统消息
  WillProcessMessage(msg);//处理前给观察者通知
  ...
  DidProcessMessage(msg);//处理后给观察者通知
  return true;
}
...
//我们刚刚暂时丢弃了一个kMsgHaveWork消息
  MSG msg;
  bool have_message = (0 != PeekMessage(&msg, NULL, 0, 0, PM_REMOVE));//从队列里移除kMsgHaveWork之后的那个消息(可能是wm_timer)
  //kMsgHaveWork之后的那个消息不可能还是kMsgHaveWork，因为通过have_work_
  //我们确保消息队列里只有一个kMsgHaveWork，参见ScheduleWork
  DCHECK(!have_message || kMsgHaveWork != msg.message ||
         msg.hwnd != message_hwnd_);

  // Since we discarded a kMsgHaveWork message, we must update the flag.
  int old_have_work = InterlockedExchange(&have_work_, 0);//丢弃了那个消息暂时将have_work_标志为0
  ...
  ScheduleWork();//复原我们的kMsgHaveWork消息，将kMsgHaveWork重新添加到消息队列中
  return ProcessMessageHelper(msg);//递归:解析kMsgHaveWork之后的那个消息
}
