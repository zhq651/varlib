...
MessageLoop::MessageLoop(Type type)
    : type_(type),
      ...
      //一个线程只能用一个MessageLoop
  DCHECK(!current()) << "should only have one message loop per thread";
      //设置当前线程的MessageLoop为创建的对象 
...
MessageLoop::~MessageLoop() {
  ...
  //删除task可能会额外的添加一些task进来
  bool did_work;
  for (int i = 0; i < 100; ++i) {
    DeletePendingTasks();
    ReloadWorkQueue();
    // If we end up with empty queues, then break out of the loop.
    //返回true说明我干活了，删除了一些task,同时说明后面可能还有活要干
    did_work = DeletePendingTasks();
    //没活干了跳出循环
    if (!did_work)
      break;
  }
  DCHECK(!did_work);//确保队列里面为空，已经没活干了
...
//将incoming_queue_lock_队列放到工作队列work_queue_中，只有当work_queue_为空的
//时候才会加锁将incoming_queue_lock_输出到work_queue_队列中
...
bool MessageLoop::DeletePendingTasks() {
    //任意一个任务队列为非空就会返回true,表示我做了一些删除task的工作
